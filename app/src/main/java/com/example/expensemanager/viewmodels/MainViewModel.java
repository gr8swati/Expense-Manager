    ...
    public void getTransactions(Calendar calendar , String type) {
        ...
        if (Constants.SELECTED_TAB_STATS == Constants.DAILY) {
            ...
            newTransactions = realm.where(Transaction.class)
                    ...
                    .lessThanOrEqualTo("date", new Date(calendar.
                            getTime().getTime() + (24 * 60 * 60 * 1000))).equalTo("type", type).findAll();
 ...
 public void getTransactions(Calendar calendar) {
        ...
        if (Constants.SELECTED_TAB == Constants.DAILY) {
            ...
            newTransactions = realm.where(Transaction.class)
                    .greaterThanOrEqualTo("date", calendar.getTime())
                    .lessThanOrEqualTo("date", new Date(calendar.
                            getTime().getTime() + (24 * 60 * 60 * 1000))).findAll();
             ...
             total = realm.where(Transaction.class)
               ...
               newTransactions = realm.where(Transaction.class)
                    .greaterThanOrEqualTo("date", startTime)
                    .lessThanOrEqualTo("date", endTime).findAll();
...
//
//        realm.beginTransaction();
//        realm.copyToRealmOrUpdate(new Transaction("INCOME", "Other", "Rent", "note here", new Date(), 500, new Date().getTime()));
//        realm.copyToRealmOrUpdate(new Transaction("EXPENSE", "Wallet", "Bills", "note here", new Date(), 5000, new Date().getTime()));
//        realm.copyToRealmOrUpdate(new Transaction("INCOME", "Cash", "Investment", "note here", new Date(), 200, new Date().getTime()));
//        realm.copyToRealmOrUpdate(new Transaction("EXPENSE", "Bank", "Salary", "note here", new Date(), 900, new Date().getTime()));
//        realm.commitTransaction();
//    }
    void updateDataBase(){
